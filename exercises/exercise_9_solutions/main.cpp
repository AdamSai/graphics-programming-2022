...
int main()
{
    ...
    while ( !glfwWindowShouldClose( window ))
    {
        ...
        {
            glm::vec4 rotatedLight =
                    glm::rotate( glm::mat4( 1.0f ), lightRotationSpeed * deltaTime, glm::vec3( 0.0f, 1.0f, 0.0f )) *
                    glm::vec4( config.lights[1].position, 1.0f );
        ...
        if ( postFXMode == PostFXMode::Realistic )
        {
            ...
            if ( tempBuffers[0] != 0 )
            {
                ...
                shader->setFloat( "threshold", config.bloomThreshold );
                shader->setFloat( "scale", config.bloomScale );
                shader->setFloat( "maxIntensity", config.bloomMax );
        ...
        } else if ( postFXMode == PostFXMode::CelShading )
        {
            // TODO 9.5 : Cel-shading pass
            {
                ...
                shader->setFloat( "exposure", config.exposure );
                shader->setInt( "steps", config.celshadingSteps );
            ...
            // TODO 9.6 : Add the outline pass
            {
                ...
                shader->setVec2( "scale", glm::vec2( 1.0f / width, 1.0f / height ));
                shader->setVec3( "outlineColor", config.outlineColor );
                shader->setFloat( "distance", config.outlineDistance );

                glEnable( GL_BLEND );
                glBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
                drawFullscreenPass( "SourceTexture", gDepth );
                glDisable( GL_BLEND );
            }
        } else
        {
...
void drawGui()
{
...

    {
        ...
        ImGui::SliderFloat( "exposure", &config.exposure, 0.01f, 10.0f );
        ImGui::Separator();
        ...
        ImGui::Separator();
        ImGui::SliderFloat( "bloom threshold", &config.bloomThreshold, 0.0f, 10.0f );
        ImGui::SliderFloat( "bloom scale", &config.bloomScale, 0.0f, 2.0f );
        ImGui::SliderFloat( "bloom max intensity", &config.bloomMax, 0.0f, 10.0f );
        ImGui::Separator();
        ImGui::SliderInt( "cel-shading steps", &config.celshadingSteps, 1, 10 );
        ImGui::ColorEdit3( "outline color", (float *) &config.outlineColor );
        ImGui::SliderFloat( "outline distance", &config.outlineDistance, 0.001f, 0.1f );
        ImGui::Separator();

        ImGui::Text( "Shading model: " );
        {
            if ( ImGui::RadioButton( "Realistic PostFX", postFXMode == PostFXMode::Realistic ))
            { postFXMode = PostFXMode::Realistic; }
            if ( ImGui::RadioButton( "Cel-Shading PostFX", postFXMode == PostFXMode::CelShading ))
            { postFXMode = PostFXMode::CelShading; }
            if ( ImGui::RadioButton( "No PostFX", postFXMode == PostFXMode::None ))
            { postFXMode = PostFXMode::None; }
        }
...
void setLightUniforms( Light &light, Camera *viewSpace )
{
    ...
    if ( viewSpace )
    {
        glm::vec4 viewSpacePosition =
                viewSpace->GetViewMatrix() * glm::vec4( light.position, light.radius > 0.0f ? 1.0f : 0.0f );
...
void drawShadowMap()
{
    ...
    glm::mat4 lightView = glm::lookAt( glm::normalize( config.lights[0].position ) * shadowMapDepthRange * 0.5f,
