...
unsigned int
createVertexArray( std::vector<float> &positions, std::vector<float> &colors, std::vector<unsigned int> &indices );
...
// --------------
void updateShaderTransform( glm::mat4 transform );
...
void drawPlane()
{
    ...
    glm::mat4 transform = translation * rotation * planeLeaning * scale;

    updateShaderTransform( transform );
    ...
    // left wing
    auto leftTransform = glm::scale( transform, { -1, 1, 1 } );
    updateShaderTransform( leftTransform );
    drawSceneObject( planeWing );

    // rear back wing
    transform = glm::translate( transform, { 0, -0.5f, 0 } );
    transform = glm::scale( transform, { 0.5f, 0.5f, 0.5f } );
    updateShaderTransform( transform );
    drawSceneObject( planeWing );

    // rear back wing
    leftTransform = glm::translate( leftTransform, { 0, -0.5f, 0 } );
    leftTransform = glm::scale( leftTransform, { 0.5f, 0.5f, 0.5f } );
    updateShaderTransform( leftTransform );
    drawSceneObject( planeWing );

    // Propeller
    transform = glm::translate( transform, { 0, 2.0f, 0 } );
    transform = glm::rotate( transform, 180.0f, { 1, 0, 0 } );
    transform = glm::rotate( transform, currentTime * 10, { 0, 0, 1 } );
    updateShaderTransform( transform );
